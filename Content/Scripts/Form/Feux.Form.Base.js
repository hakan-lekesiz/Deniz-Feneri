Feux.Form = {
    ...
    UX: {
        ...
        checkValidationAndSubmitForm: function (arg) {
            ...
            if (form.querySelector(".g-recaptcha").id === "newsletterRecaptcha") {
                recaptcha = grecaptcha.getResponse(nRecaptcha);
            }
            else if (form.querySelector(".g-recaptcha").id === "contactRecaptcha") {
                recaptcha = grecaptcha.getResponse(cRecaptcha);
            }
            else if (form.querySelector(".g-recaptcha").id === "volunteerRecaptcha") {
                recaptcha = grecaptcha.getResponse(cRecaptcha);
            }

            if (recaptcha === "") {
                event.preventDefault();
                form.querySelector(".g-recaptcha div div iframe").style.border = "1px solid red";
        ...
        submitConfirmCode: function (sender) {
            //server side confirm code gönderilir backend ile iletişim. gelen cevaba göre aşağıdaki kodlara devam edilmelidir.


            //daha sonra ilgili input açılır
            document.getElementById("sms-password").removeAttribute("style");

            //submit fonskiyonu değiştirlir çünkü tekrar submitConfirmCode fonks. çalışmayacak login olma fonskiyonu çağrılacak
            sender.setAttribute("onclick", "Feux.Form.UX.login({ev:event,sender:this});");
...
var newsletterRecaptchaCallback = function (response) {
    if (response !== "") {
        document.querySelector("#newsletterRecaptcha div div iframe").style.border = "none";
    }
};

var contactRecaptchaCallback = function (response) {
    if (response !== "") {
        document.querySelector("#contactRecaptcha div div iframe").style.border = "none";
    }
};

var volunteerRecaptchaCallback = function (response) {
    if (response !== "") {
        document.querySelector("#volunteerRecaptcha div div iframe").style.border = "none";
    }
};

var CaptchaCallback = function () {
     
    var newsletterRecaptcha = document.getElementById("newsletterRecaptcha");
    var contactRecaptcha = document.getElementById("contactRecaptcha");
    var volunteerRecaptcha = document.getElementById("volunteerRecaptcha");
    
    if (contactRecaptcha) {
        cRecaptcha = grecaptcha.render('contactRecaptcha', {
            'sitekey': "6LdqFW8eAAAAAMKM-WRBxRvTYQWdh5K_xNW7xMqo",
            'theme': 'light',
            'callback': contactRecaptchaCallback,
        });
    }
    if (newsletterRecaptcha) {
        nRecaptcha = grecaptcha.render('newsletterRecaptcha', {
            'sitekey': "6LdqFW8eAAAAAMKM-WRBxRvTYQWdh5K_xNW7xMqo",
            'theme': 'light',
            'callback': newsletterRecaptchaCallback,
        });
    }
    if (volunteerRecaptcha) {
        vRecaptcha = grecaptcha.render('volunteerRecaptcha', {
            'sitekey': "6LdqFW8eAAAAAMKM-WRBxRvTYQWdh5K_xNW7xMqo",
            'theme': 'light',
            'callback': volunteerRecaptchaCallback,
        });
    }
};